...
export function EditStaffDrawer({ staff, isOpen, onClose, onSuccess }: EditStaffDrawerProps) {
  ...
  async function onSubmit(values: EditStaffFormData) {
    ...
    try {
      ...
      const result = await dispatch(updateStaff({ id: staff.id, data: updateData })).unwrap()
      
      // Use backend message if available, otherwise use custom message
      const successMessage = result?.message || `${staff.user?.first_name} ${staff.user?.last_name}'s information has been successfully updated.`
      ...
      if (errorMessage === "Failed to update staff member") {
