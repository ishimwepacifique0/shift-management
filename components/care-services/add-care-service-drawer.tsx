...
import { createCareService, clearError } from "@/feature/careServices/careServiceSlice"
import { usePriceBooks } from "@/hooks/use-price-books"
import { useServiceTypes } from "@/hooks/use-service-types"

const addCareServiceSchema = z.object({
...
type AddCareServiceFormData = z.infer<typeof addCareServiceSchema>

interface AddCareServiceDrawerProps {
  isOpen: boolean
  onClose: () => void
}

export function AddCareServiceDrawer({ isOpen, onClose }: AddCareServiceDrawerProps) {
  ...
  const form = useForm<AddCareServiceFormData>({
    resolver: zodResolver(addCareServiceSchema),
    defaultValues: {
      name: "",
      description: "",
      price_book_id: "",
  ...
  const onSubmit = async (data: AddCareServiceFormData) => {
    try {
      ...
      const result = await dispatch(createCareService(careServiceData))
      
      if (createCareService.fulfilled.match(result)) {
        toast({
          title: "Success",
          description: "Care service created successfully",
        })
        form.reset()
        onClose()
      } else if (createCareService.rejected.match(result)) {
        toast({
          title: "Error",
          description: result.payload as string || "Failed to create care service",
          variant: "destructive",
        })
      }
    } catch (error: any) {
      console.error("Error creating care service:", error)
      toast({
        title: "Error",
        description: error?.message || "Failed to create care service",
        variant: "destructive",
      })
    }
  }
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="w-[400px] sm:w-[540px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Add New Care Service</DialogTitle>
          <DialogDescription>
            Create a new care service for your company. This will be available for shift scheduling.
          </DialogDescription>
        </DialogHeader>
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            ...
            <DialogFooter>
              ...
              <Button type="submit" disabled={status === "loading"}>
                {status === "loading" ? "Creating..." : "Create Care Service"}
              </Button>
            </DialogFooter>
          </form>
        </Form>
