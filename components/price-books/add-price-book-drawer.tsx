...
import { createPriceBook, clearError } from "@/feature/priceBooks/priceBookSlice"
import { useServiceTypes } from "@/hooks/use-service-types"

const addPriceBookSchema = z.object({
...
type AddPriceBookFormData = z.infer<typeof addPriceBookSchema>

interface AddPriceBookDrawerProps {
  isOpen: boolean
  onClose: () => void
}

export function AddPriceBookDrawer({ isOpen, onClose }: AddPriceBookDrawerProps) {
  ...
  const form = useForm<AddPriceBookFormData>({
    resolver: zodResolver(addPriceBookSchema),
    defaultValues: {
      name: "",
      rate_per_hour: 0.01,
      service_type_id: "",
  ...
  const onSubmit = async (data: AddPriceBookFormData) => {
    try {
      const priceBookData = {
        name: data.name.trim(),
      ...
      const result = await dispatch(createPriceBook(priceBookData))
      ...
      if (createPriceBook.fulfilled.match(result)) {
        ...
        form.reset()
        onClose()
      } else if (createPriceBook.rejected.match(result)) {
        toast({
          title: "Error",
          description: result.payload as string || "Failed to create price book",
          variant: "destructive",
        })
      }
    } catch (error: any) {
      ...
      toast({
        title: "Error",
        description: error?.message || "Failed to create price book",
        variant: "destructive",
      })
    }
  }
    ...
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="w-[400px] sm:w-[540px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Create a new pricing rate for your company. This will be available for care services and shifts.
          </DialogDescription>
        </DialogHeader>
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            ...
            <DialogFooter>
              ...
              <Button type="submit" disabled={status === "loading"}>
                {status === "loading" ? "Creating..." : "Create Price Book"}
              </Button>
            </DialogFooter>
          </form>
        </Form>
